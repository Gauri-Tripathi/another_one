...
import { BarChart3, Cloud, Pause, PawPrint, Play, Bell, Target, Timer, Settings as SettingsIcon } from 'lucide-react';
...
const DEFAULT_SETTINGS = { catId:localStorage.getItem('purrductive-cat')||'silver',idleThresholdSeconds:60,breakIntervalSeconds:7200,launchAtLogin:true,sync:{url:'',key:'',companionUrl:''} };
...
export default function App() {
  ...
  const todayProductive=useMemo(()=>summarize(segmentsForDay(local,localDay())).productive,[local]);
  async function previewCat(){if(window.purrductive?.previewBreak){try{await window.purrductive.previewBreak();}catch(e){setError(e.message);}}else setPreview(true);}
  ...
  if (preview) return <BreakScreen preview catId={settings.catId} onClose={() => setPreview(false)}/>;
  ...
  return <div className="app-shell">
    <aside><div className="brand-mark"><PawPrint/><span>purrductive</span></div><nav aria-label="Main navigation">{[['dashboard','Your day',BarChart3],['reminders','Reminders',Bell],['goals','Goals',Target],['focus','Focus & breaks',Timer],['settings','Settings & sync',SettingsIcon]].map(([key,label,Icon])=><button key={key} className={page===key?'active':''} aria-current={page===key?'page':undefined} onClick={()=>setPage(key)}><Icon/>{label}</button>)}</nav><div className="sidebar-note"><PawPrint/><h3>Small steps.<br/>Big stretches.</h3><p>Your time is information, not a report card.</p></div><div className={'local-badge '+(live?.paused ? 'paused' : '')}><span/><div><b>{live ? live.paused ? 'Tracking paused' : 'Tracking locally' : 'Mobile companion'}</b><small>{live?.activeApp || 'Last uploaded activity'}</small></div></div></aside>
    <main className="dashboard"><header className="topbar"><div><p className="eyebrow">A LITTLE MORE INTENTIONAL</p><h1>{page==='dashboard'?`Make room for your day${name?', '+name:''}.`:page==='reminders'?'Your little reminders.':page==='goals'?'Your goals, one step at a time.':'Focus, then stretch.'}</h1></div>{live && <button className="tracking-button" onClick={async () => {try {const paused=await window.purrductive.toggleTracking();setLive(v => ({...v,paused}));} catch(e) {setError(e.message);}}}>{live.paused ? <Play size={16}/> : <Pause size={16}/>} {live.paused ? 'Resume' : 'Pause'}</button>}</header>
      {(page==='reminders'||page==='goals')&&<Planner key={page} productiveSeconds={todayProductive} section={page}/>}
      {page==='focus'&&<><CatCompanion live={live} onPreview={previewCat} catId={settings.catId} onCatChange={async catId=>{try{const next={...settings,catId};if(desktop)await window.purrductive.saveSettings(next);else localStorage.setItem('purrductive-cat',catId);setSettings(next);}catch(e){setError(e.message);}}}/>{error&&<p role="alert">{error}</p>}<FocusTimer/></>}
      {page==='dashboard'&&<>
      ...
      {live?.recognitionStatus&&<div className="recognition-status"><span>{live.recognitionStatus}</span><button className="text-button" onClick={()=>setPage('settings')}>Recognition settings →</button></div>}
      {live?.trackingHealth&&<details className="tracking-health"><summary>Tracking diagnostics · {live.paused?'paused':live.activeApp==='Idle'?'idle':'fast sampling every ~250 ms'}</summary><p>Since launch: {live.trackingHealth.accepted} stable samples; {live.trackingHealth.rejected} skipped reads. Last read took {live.trackingHealth.lastReadMs??'—'} ms.</p><p>Last stable observation: {live.trackingHealth.lastObservedAt?new Date(live.trackingHealth.lastObservedAt).toLocaleTimeString():'not yet available'}. {live.websiteStatus}</p><p>Fractional seconds are preserved. Browser address lookups and AI run separately from the time counter. Switches between samples remain estimates; missed reads and gaps over 3 seconds are not filled in.</p></details>}
      <AppearanceStudio onPreview={previewCat}/>
